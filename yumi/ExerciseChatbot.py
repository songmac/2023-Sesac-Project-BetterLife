...
class ExerciseChatbot:
    ...
    def ask_next_question(self):
        if self.current_question_index < len(self.questions):
            ...
            #코사인 유사도로 시설정보 가져옴
            print(user_input)
            user_answers = ' '.join(answer for answer in user_input)
            recommendations  = get_facility_info.recommend_programs(user_answers, data)
            completion_message = "질문을 마쳤습니다. 아래와 같은 프로그램을 추천해드리겠습니다."
            chat_history_panel.object += f"\n시스템: {completion_message}"
            chat_history_panel.object += f"\n시스템: {recommendations}"
